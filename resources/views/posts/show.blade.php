                ...
                <div class="col-lg-8">
                    @auth
                        @canany(['update','delete'], $post)
                            <div class=" row mb-4">
                                 <a class="mr-2 btn btn-sm btn-outline-dark" href="{{ route('posts.edit',['post' => $post->id]) }}">
                                     O'zgartirish
                                </a>
                                 ...
                                 <form action="{{ route('posts.destroy',['post' => $post->id]) }}" method="POST"
                                    onsubmit="return confirm('Rostan ham o\'chirishni xoxlaysizmi');"> 
                                     @csrf
                                     @method('DELETE')
                                     <button type="submit" class="mr-2 btn btn-sm btn-outline-danger">
                                         O'chirish
                                     </button>
                                 </form>
                            </div>
                    ...
                    <div class="mb-5">
                        <div class="d-flex mb-2">
                            ...
                            <div>
                            <a class="bg-success rounded-5 p-1 text-uppercase text-white font-weight-medium" href="">{{ $post->category->name }}</a>
                            </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="media mb-4">
                            ...
                            <div class="media-body">
                                <h6>{{ $comment->user->name }} <small><i>{{ $post->created_at }}</i></small></h6>
                                <p>{{ $comment->body }}</p>
                        ...
                        <ul class="list-inline m-0">
                            @foreach($categories as $category)
                                <li class="mb-1 py-2 px-3 bg-light d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge badge-primary badge-pill">              {{$category->posts->count()}}
                                    </span>
                                </li>
                            @endforeach
                        </ul>
